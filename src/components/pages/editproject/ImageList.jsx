...
import AddProjectImages from '../../modal/projectimages/AddProjectImages'
import EditProjectImages from '../../modal/projectimages/EditProjectImages'
...
function ImageList({ id }) {
  ...
  // Images add modals
  const [is_ImgModal, setImgModal] = useState(false)
  const [is_ImgEditModal, setImgEditModal] = useState(false)
  const [editImgData,setImgEditdata] = useState('')
  ...
  return (
    <Section>
      <ImageLising>
        <div className='w-[95%] mx-auto grid grid-cols-2'>
          ...
          <Button className='flex justify-end items-start'>
           <button onClick={()=>setImgModal(true)}>Add Images</button>
          </Button>
        </div>
        {projectImages === null || isLoading? (
          <Loader>
      ...
      <FloorPlanList>
        ...
        {floorPlanImages === null || isLoading? (
          <Loader>
            <Spinner className="h-6 w-6" />
          </Loader>
        ) : (
      ...
      <AddProjectImages isModal={is_ImgModal} setModal={setImgModal} projectId={id} fetData={fetData}/>
      {is_ImgEditModal && editImgData && <EditProjectImages isModal={is_ImgEditModal} setModal={setImgEditModal} fetData={fetData} editImgData={editImgData}/>}
